...
<!-- Haut de page -->
<head>
  <title>Photos de Trois-Rivières - Auberge Internationale de Trois-Rivières</title>
    ...
    <div id="header">
	  ...
	  <a href="http://www.facebook.com/pages/Auberge-de-jeunesse-HI-Trois-Rivi%C3%A8res/180609951984544"><img id="facebook_logo" src="style/facebook.png" alt="facebook"/></a>
        <ul id="menu"> 
		  ...
		  <li class="tab_selected"><a href="multimedia_auberge.php">Galerie</a>
		  <ul id="menu_dropdown">
  ...
  <li id="menu_dropdown"><a href="multimedia_tr.php">Photos de Trois-Rivières</a></li>
    ...
    <div id="site_content">
      ...
      <div id="content">
        ...
        <h1 id="titre_photo">Photos de Trois-Rivières</h1>
		<p><br/></p>
		<div id="images_multi">
			
		<?php
			//Requête
			$reponse = $bdd->query('SELECT * FROM multimedia WHERE section_multi = "TR"');
			$nbr = $bdd->query('SELECT * FROM autres');
			//Le nombre de photos par page
			$picture_per_page = 0; // valeur par défaut
			$nbr_page = 1;
			$page = 1;
			
			while ($nbr_pictures = $nbr->fetch())
			{
				$picture_per_page = $nbr_pictures['nbr_pictures_tr'];
			}
			//Permet de savoir comment il y a de photos dans la section Auberge
			$reponse = $bdd->query('SELECT COUNT(*) FROM multimedia WHERE section_multi = "TR"') or die("error reading database");
			...
			$query = $bdd->query("SELECT * FROM multimedia WHERE section_multi = 'TR' LIMIT $limit_min, $limit_length;") or die("error reading database");
			...
			while ($donnees = $query->fetch())
				{
					...
					?>
					<a href="<?php echo $donnees['lien_image_multi'];?>" rel="shadowbox[image]" title="<?php echo $donnees['nom_image_multi'];?>"><img id="redimensionnement" src="<?php echo $donnees['lien_image_multi'];?>" alt="<?php $donnees['nom_image_multi'];?>"/></a>
					<?php
				}
			?>
		...
		<!-- Section du bas pour changer de page -->
			<div id="page_bas">
				<?php
					...
					?>
					<a href="multimedia_tr.php?action=galerie&page=<?php echo $i; ?>"><?php echo $i?></a>
					<?php
					}
					echo ", ";
					}
