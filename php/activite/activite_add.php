<?php
	...
	|| ($_FILES["file"]["type"] == "image/pjpeg") or EMPTY($_POST['file']))
	{
		...
		if (!EMPTY($_POST['nom']) and !EMPTY($_POST['desc']))
		{
			...
			{
				//Requête
				mysql_query("INSERT INTO activite VALUES(null,'$nom','$desc', 'multimedia/activite/$image', '$nom_lien', '$lien')") or die("La connexion a échoué.");
			}
			else
			{
				mysql_query("INSERT INTO activite (titre_activite, desc_activite, image_activite) VALUES('$nom','$desc', 'multimedia/activite/$image')") or die("La connexion a échoué.");	
			}
	...
	header("location:../../admin/activites/admin_activite_add.php");
?>