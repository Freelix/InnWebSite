  ...
  <li id="menu_dropdown"><a href="../../multimedia_tr.php">Photos de Trois-Rivières</a></li>
    ...
    <div id="site_content">
      ...
      <div id="content">
		...
		<h1 class="blue">Photos de Trois-Rivières</h1>
			
			<?php
				$query = $bdd->query('SELECT COUNT(*) FROM multimedia WHERE section_multi = "TR"');
				while ($reponse2 = $query->fetch())	
				{
					$nbr_photos = $reponse2[0];
				}
			?>
		...
		<table id="tab_reserv">
			<?php if ($nbr_photos > 1) { ?>
				...
				?>
				<tr>
					<td><img id="img_tab_taille" src="../../<?php echo $data['lien_image_multi'];?>" alt="<?php echo $data['nom_image_multi'] ?>"/></td>
					<td><a href="../../php/multimedia/multi_image_supp.php?id=<?php echo $data['no_multi'];?>&section=<?php echo $data['section_multi'];?>"><img id="btn_edit_supp" src="../../style/supprimer.gif" alt="supprimer"/></td>
				<?php }
				else if ($nbr % 2 === 0)
				{ ?>
					<td><img id="img_tab_taille" src="../../<?php echo $data['lien_image_multi'];?>" alt="<?php echo $data['nom_image_multi'] ?>"/></td>
		...
		<h1 class="blue">Ajout</h1>
		<form name="input" action="../../php/multimedia/tr_add.php" method="post" enctype="multipart/form-data">
			<h2>Nom Français</h2>
			<input id="titre_design" type="text" name="nom" value="<?php if (isset($_SESSION['nom'])){ echo $_SESSION['nom'];}?>"/>
			
			<h2>Nom Anglais</h2>
			<input id="titre_design" type="text" name="nom_en" value="<?php if (isset($_SESSION['nom_en'])){ echo $_SESSION['nom_en'];}?>"/>
