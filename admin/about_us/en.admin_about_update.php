  ...
  <li id="menu_dropdown"><a href="../../multimedia_tr.php">Photos de Trois-Rivières</a></li>
    ...
    <div id="site_content">
      ...
      <div id="content">
		...
		<form name="input" action="../../php/employe/en.employe_mod.php?id=<?php echo $id;?>" method="post" enctype="multipart/form-data">
			<h2>Nom</h2>
			<?php
				$id = $_GET['id'];
				$reponse = $bdd->query("SELECT * FROM en_employe WHERE no_employe = $id");
				while ($data = $reponse->fetch())
			{ ?>
			<input id="titre_design" type="text" name="nom" value="<?php echo $data['nom_employe']; ?>"/>
			
			<h2>Informations</h2>
			<textarea id="area_design" name="desc"><?php echo $data['desc_employe']; ?></textarea>
